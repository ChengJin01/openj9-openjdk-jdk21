...
public final class SystemLookup implements SymbolLookup {
    ...
    private static MemorySegment getInlinedFunctListAddr() {
         ...
         long FuncSymbolsSize = OperatingSystem.isAix()
                 ? AixFuncSymbols.LAYOUT.byteSize()
                 : ZosFuncSymbols.LAYOUT.byteSize();
        ...
        MemorySegment funcs_addr = funcsLibLookup.find("funcs_addr").orElseThrow();
        MethodHandle handle = Linker.nativeLinker().downcallHandle(funcs_addr, FunctionDescriptor.of(ADDRESS));
        try {
            funcsAddr = ((MemorySegment)handle.invoke()).reinterpret(FuncSymbolsSize);
        } catch (Throwable e) {
